...
class Article extends Controller
{
            ...
            if(count($a_skus) > 1 && $i_countProducts==count($a_skus)) {
                ...
                for ($j=0; $j < count($a_skus); $j++) {
                    ...
                    if(($j+1)==count($a_skus)) {
                        ...
                        $a_parentProduct = $products;
                        $a_parentProduct[0]['sku'] = $s_parentTempSku;
                        $a_parentProduct[0]['_type'] = 'configurable';
                        $a_parentProduct[0]['_attribute_set'] = 'Configurable Product';
                        $a_parentProduct[0]['visibility'] = 4;
                        $a_parentProduct[0]['is_in_stock'] = 1;
                        $a_parentProduct[0]['qty'] = 100;
                        if($i_languageId==99)
                        {
                            $price=0;
                            $price = ($a_prices[0] + (isset($article['Shipping_Costs'])?$article['Shipping_Costs']:9) ) / $a_countryInfo[0]['exchangerate_eur_multi']; 
                            $a_parentProduct[0]['price'] = $price / $this->a_shops[$i_languageId]['mwst'];
                        } else {
                            $a_parentProduct[0]['price'] = ($a_prices[0] + (isset($article['Shipping_Costs'])?$article['Shipping_Costs']:9) ) / $this->a_shops[$i_languageId]['mwst'] ; 
                        }
                        ...
                        for ($i=0; $i < count($a_images); $i++) { 
                            ...
                            if($i==0) {
                                $a_parentProduct[0]['small_image'] =  $s_MainImage;
                                $a_parentProduct[0]['thumbnail'] =  $s_MainImage;
                                $a_parentProduct[0]['image'] = $s_MainImage;
                                $a_parentProduct[0]['_media_attribute_id'] = 88;
                                $a_parentProduct[0]['_media_image'] = $s_MainImage;
                                $a_parentProduct[0]['_media_is_disabled'] = 0;
                                $a_parentProduct[0]['_media_lable'] = $article['Titel'].' - 1';
                                $a_parentProduct[0]['_media_position'] = 1;
                            } else {
                                $a_parentProduct[] = array(
                                    '_media_attribute_id' => 88,
                                    '_media_image' => $s_MainImage,
    ...
    public function updateProduct($i_languageId = null, $i_productId=null)
    {
        ...
        foreach ($a_Articles as $article) {
            ...
            if($i_languageId==99)
            {
                $price = $article['Low_Price']!=null ? $article['Low_Price'] / $a_countryInfo[0]['exchangerate_eur_multi'] 
                : ($article['First_Price'] + (isset($article['Shipping_Costs'])?$article['Shipping_Costs']:9) ) / $a_countryInfo[0]['exchangerate_eur_multi'];
                $price = (number_format(round(($price),0)-0.05,2))/$this->a_shops[$i_languageId]['mwst'];
            } else {
                $price = $article['Low_Price']!=null ? $article['Low_Price'] / $this->a_shops[$i_languageId]['mwst'] : 
                $article['First_Price'] + (isset($article['Shipping_Costs'])?$article['Shipping_Costs']:9);
            }
            #$price = (($i_languageId==99 ? $article['First_Price'] / $a_countryInfo[0]['exchangerate_eur_multi'] :  ($article['Low_Price']!=null ? $article['Low_Price']/$this->a_shops[$i_languageId]['mwst'] : $article['First_Price'])) + ($article['Low_Price']!=null ?'':(isset($article['Shipping_Costs'])?$article['Shipping_Costs']:9)));
            $products[] = array(
            ...
            if(count($a_skus) > 1 && $i_countProducts==count($a_skus)) {
                ...
                for ($j=1; $j <= count($a_skus); $j++) {
                    ...
                    if($j==count($a_skus)) {
                        ...
                        $a_parentProduct[0]['status'] = 2;
                        foreach ($a_Auslauf_status as $key => $value) {

                            if($value==0){$a_parentProduct[0]['status'] = 1;}
                        }
                        
                        if($i_languageId==99)
                        {
                            $price=0;
                            $price = $a_prices[0] / $a_countryInfo[0]['exchangerate_eur_multi']; 
                            $a_parentProduct[0]['price'] = (number_format(round(($price),0)-0.05,2)) / $this->a_shops[$i_languageId]['mwst'];
                        } else {
                            $a_parentProduct[0]['price'] = $a_prices[0]  / $this->a_shops[$i_languageId]['mwst'] ; 
                        }
                        ...
                        for ($i=0; $i < count($a_skus); $i++) {
                            ...
                            if($i_languageId==99)
                            {
                                $price_x = $a_prices[0] / $a_countryInfo[0]['exchangerate_eur_multi']; 
                                $price_x = (number_format(round(($price_x),0)-0.05,2)) / $this->a_shops[$i_languageId]['mwst'];
                                $price_y = $a_prices[$i] / $a_countryInfo[0]['exchangerate_eur_multi']; 
    ...
    function updateImagesNew($i_languageId,$i_isParent=true) 
    {
        if(!isset($i_languageId)) {die('please set the language id.');}

        $articleModel = $this->loadModel('ArticleModel');
        $a_skus = array('2524-uk','2528-uk','2529-uk','2530-uk','2531-uk','2533-uk','2534-uk','2540-uk','2547-uk','2548-uk','2549-uk','2551-uk','2553-uk','2554-uk','2555-uk','2556-uk','2557-uk','2558-uk','2559-uk','2564-uk','2565-uk','2566-uk','2567-uk','2568-uk','2569-uk','2572-uk','2584-uk','2588-uk','2608-uk','2624-uk','2631-uk','2633-uk','2634-uk','2635-uk','2636-uk','2637-uk','2638-uk','2639-uk','2640-uk','2645-uk','2646-uk','2657-uk','2661-uk','2663-uk','2664-uk','2671-uk','2672-uk','2673-uk','2674-uk','2675-uk','2676-uk','2679-uk','2693-uk','2709-uk','2710-uk','2714-uk','2731-uk','2738-uk','2744-uk','2749-uk','2754-uk','2755-uk','2757-uk','2758-uk','2759-uk','2760-uk','2762-uk','2769-uk','2777-uk','2779-uk','2781-uk','2783-uk','2793-uk','2799-uk','2814-uk','2819-uk','2820-uk','2832-uk','2843-uk','2864-uk','2870-uk','2873-uk','2875-uk');
        foreach($a_skus as $sku) {
            $a_Articles = $articleModel->getImages(substr($sku,0,-3), $i_isParent, $i_languageId);
            #print_r($a_Articles);die;
            foreach ($a_Articles as $article) {
                #echo "<pre>";
            #print_r($article);
            #echo "<pre>";
